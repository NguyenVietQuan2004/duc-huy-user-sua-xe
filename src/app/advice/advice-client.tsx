...
export default function AdviceClient({ adviceList, banner }: Props) {
  const [currentPage, setCurrentPage] = useState(1);
  const totalPages = Math.ceil(adviceList.length / ITEMS_PER_PAGE);
  const currentItems = adviceList.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
      ...
      <div className="max-w-[1100px] px-4 mx-auto text-[18px] py-20 flex flex-col gap-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {currentItems.map((advice) => {
            const date = new Date(advice.created_at);
            ...
            return (
              ...
              >
                ...
                <div className="flex gap-4 mb-1 mt-auto items-start">
                  ...
                  <div
                    className="text-sm text-gray-700 leading-snug line-clamp-4 group-hover:text-[#f8ab34] transition-all duration-200"
