        ...
        {sampleArrayBlog.map((item, index) => {
          ...
          return (
            <SwiperSlide key={item.title}>
              <Link href={`/chuongtrinhkm/${item._id}`}>
                ...
                <div className="p-8 flex flex-col gap-4 select-none z-10 relative ">
                  ...
                  <div className="text-[#808080] text-sm">{item.created_at.toDateString()}</div>
                  <div className="font-light line-clamp-3">{item.content}</div>
                </div>
              </Link>
            </SwiperSlide>
          );
